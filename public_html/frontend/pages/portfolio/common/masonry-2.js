...
const MasterTabPannel = ({ data, grid, colClass }) => {
  ...
  return (
    <>
      ...
      <Masonry
        breakpointCols={grid}
        className="isotopeContainer row masonry-grid-col"
                ...
                <div className="border-portfolio">
                  <div>
                    <div
                      ...
                      onClick={() => onClickImg(item.img, index)}
                    >
        ...
        {photoIndex.isOpen && (
          <>
            <Lightbox
              ...
              nextSrc={`${data[(photoIndex.index + 1) % data.length].img}`}
              prevSrc={`${
                data[(photoIndex.index + data.length - 1) % data.length].img
              }`}
              imageTitle={`${photoIndex.index + 1}/${data.length}`}
              onCloseRequest={() =>
